...
interface StayRepository extends JpaRepository<Stay, Long> {
    ...
    @Query(value =
            "SELECT * " +
                    ...
                    "AND (:longitude IS NULL OR :maxDistance IS NULL OR ABS(s.longitude - :longitude) < :maxDistance) " +
                    "AND (:latitude IS NULL OR :maxDistance IS NULL OR ABS(s.latitude - :latitude) < :maxDistance) " +
                    "ORDER BY s.name ASC"
            , nativeQuery = true)
